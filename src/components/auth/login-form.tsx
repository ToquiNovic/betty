  ...
  return (
    <Card className="shadow-xl border border-border/80 overflow-hidden backdrop-blur-sm">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="rounded-lg bg-muted/40 p-3 text-[11px] text-muted-foreground space-y-1 border border-border/50">
          <p className="font-semibold text-foreground flex items-center gap-1.5">
            ...
            Cuentas Compartidas de la Grid:
          </p>
          <p>• <strong>Administrador:</strong> <code>admin</code> (Superadmin & Control Total)</p>
          <p>• <strong>Operador / Avatar:</strong> <code>operator</code> o tu avatar de OpenSim</p>
        </div>
        ...
        <div className="pt-2">
          ...
          >
            <span>{showLocalAuth ? 'Ocultar opciones secundarias' : 'Más opciones de acceso (Google / Local)'}</span>
        ...
        {showLocalAuth && (
          <div className="space-y-4 pt-2 border-t border-border/50 animate-in fade-in-50 duration-200">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
              ...
              <Button type="submit" variant="secondary" className="w-full h-9 text-xs font-semibold" disabled={isLoading}>
      ...
      <CardFooter className="flex justify-center border-t py-3 text-xs text-muted-foreground bg-muted/20">
        ...
        <strong className="text-foreground ml-1">Keycloak OpenID Connect</strong>
      </CardFooter>
    </Card>
  );
}
